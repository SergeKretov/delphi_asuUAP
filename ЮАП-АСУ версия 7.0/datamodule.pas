...
begin
  if sotrudnikID<>'SUPERADMIN' then
    ...
    begin
      showmessage('Удалить дело может только Суперадминистратор или Администратор офиса к которому пренадлежит дело!');
      abort();
    end else
    begin
       if messagedlg('Удалить текущее дело?',mterror, mbokcancel,0)= mrCancel then
         abort();
    end else
      if messagedlg('Удалить текущее дело?',mterror, mbokcancel,0)= mrCancel then
         abort();
...
begin
  if sotrudnikID<>'SUPERADMIN' then
    ...
    begin
      showmessage('Редактировать дело может только Суперадминистратор или Администратор офиса к которому пренадлежит дело или сотрудник, который ведет дело!');
      abort();
    end;
    end else
...
begin
  if sotrudnikID<>'SUPERADMIN' then
    ...
    begin
      showmessage('Удалить задачу может только Суперадминистратор или Администратор офиса к которому пренадлежит дело!');
      abort();
    end else
    begin
       if messagedlg('Удалить текущее дело?',mterror, mbokcancel,0)= mrCancel then
...
begin
  if sotrudnikID<>'SUPERADMIN' then
    ...
    begin
      showmessage('Редактировать задачу может только Суперадминистратор или Администратор офиса к которому пренадлежит задача или сотрудник, который выполняет задачу!');
      abort();
    end;
    end else
