...
begin
  ...
  if n0=0 then showmessage('Не выбран не один из архивов!');
  if n0>1 then showmessage('Выбран более чем 1 архив. Развернуть локально одновременно несколько архивов не возможно! Выбирите один архив и повторите попытку.') else
    for nn:=0 to form1.checklistbox1.Items.Count-1 do
      if form1.checklistbox1.Checked[nn] then
        begin
          showmessage('Будет развернут локально '+form1.checklistbox1.Items.Strings[nn]);
          flg:=0;
          fname:='';
          ...
          if bname='' then showmessage('Не могу развернуть базу. Проверьте параметры локального SQL сервера!') else
            begin
...
begin
  ...
  if FindFirst(pa+'\msqlarch\*.sql',faArchive,SearchRec) = 0 then
    repeat
      CheckListBox1.Items.Add('Архив от '+DateFromName(SearchRec.Name)+' Файл: {'+SearchRec.Name+'}');
...
if PrepearBase then
begin
showmessage('Подготовка данных завершена. Подключаюсь к серверу локальной БД.');
try
...
showmessage('Начинаю загружать архивные данные в локальную БД.');
...
showmessage('Локальная база данных готова к использованию.');
...
begin
  showmessage('(с) 2019 Сергей С. Барсуков v1.0');
end;
...
var nn,n0:integer;
    ...
    SearchRec:TSearchRec;
begin
  if MessageDlg('Будут удалены выбранные архивы. Продолжить?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin
   for nn:=0 to form1.checklistbox1.Items.Count-1 do
      if form1.checklistbox1.Checked[nn] then
        begin
      ...
      if FindFirst(pa+'\msqlarch\*.sql',faArchive,SearchRec) = 0 then
      repeat
        form1.CheckListBox1.Items.Add('Архив от '+DateFromName(SearchRec.Name)+' Файл: {'+SearchRec.Name+'}');
      until FindNext(SearchRec) <> 0;
  end;
end;
