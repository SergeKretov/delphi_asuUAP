...
{$ENDIF}
  ...
  Classes, SysUtils, DB, CRTypes, {$IFDEF FPC}MemDataSet{$ELSE}MemDS{$ENDIF},
...
{ TCustomMyConnection }
  ...
  protected
    ...
    procedure SetNullForZeroDelphiDate(const Value: boolean);
  ...
  public
    ...
    property NullForZeroDelphiDate: boolean read FNullForZeroDelphiDate write SetNullForZeroDelphiDate default False;
...
//upd    function CommitOnDisconnect: boolean; override;
  ...
  public
    ...
    procedure GetTriggerNames(List: _TStrings; AllTriggers: boolean = False);
...
{ TMyConnection }
  ...
  {$ENDIF}
    ...
    property NullForZeroDelphiDate default False;
...
{$IFDEF HAVE_OPENSSL}
  ...
  published
    property ChipherList: string read FCipherList write SetCipherList;
...
{ TCustomMyDataSet }
  ...
  published
    property SetFieldsReadOnly default False;
...
{$IFDEF VER6P}
  DateUtils,
{$ENDIF}
  CRFunctions, MemUtils, MyParser, MyDacMonitor;
...
begin
  ...
  if Dest is TCustomMyConnectionOptions then begin
    ...
    TCustomMyConnectionOptions(Dest).NullForZeroDelphiDate := NullForZeroDelphiDate;
  end;
end;
...
procedure TCustomMyConnectionOptions.SetNullForZeroDelphiDate(const Value: boolean);
begin
  if FNullForZeroDelphiDate <> Value then begin
    TCustomMyConnection(FOwner).CheckInactive;
    FNullForZeroDelphiDate := Value;
    if TCustomMyConnection(FOwner).IConnection <> nil then
      TCustomMyConnection(FOwner).IConnection.SetProp(prNullForZeroDelphiDate, Value);
  end;
end;
...
begin
  if Pooling then begin
    ...
    try
      ...
      Result := TMyConnectionPoolManager.GetConnection(ConnectionParameters,
        TCustomMySQLMonitor) as TMySQLConnection;
    finally
...
{$ENDIF}
  ...
  Result.SetProp(prNullForZeroDelphiDate, Options.NullForZeroDelphiDate);
  ...
  try
    ...
    if (Major = 5) and (Minor = 0) and (Release = 0) then
      ...
      if OldDatabase = '' then
        ...
        db_name := '''' + AnsiLowerCase(MySQLInfo.NormalizeName(OldDatabase, False, True)) + '''';
...
procedure TCustomMyConnection.GetTriggerNames(List: _TStrings; AllTriggers: boolean = False);
const
...
begin
  ...
  try
    ...
    Connect;
    if IConnection.ServerPrimaryVer < 5 then
      Exit;
    ...
    if not AllTriggers then begin
      ...
      WhereClause := 'WHERE LOWER(TRIGGER_SCHEMA) = ' + db_name;
    end
    else
      WhereClause := '';

    MyQuery.SQL.Text := Format(fmtGetTriggersSQL, [WhereClause]);

    MyQuery.FetchAll := True;
    MyQuery.Open;
    while not MyQuery.Eof do begin
      Name := _VarToStr(MyQuery.Fields[1].Value);
      if AllTriggers then begin
        s := _VarToStr(MyQuery.Fields[0].Value);
        if s <> '' then
          Name := s + '.' + Name;
      end;
      List.Add(Name);
...
begin
  ...
  try
    ...
    MetaData.Restrictions.Add('table_name=' + MySQLInfo.NormalizeName(TableName));
    MetaData.Open;
    List.Clear;
...
begin
  ...
  if Dest is TMyConnectionOptions then begin
    ...
    TMyConnectionOptions(Dest).CheckBackslashes := CheckBackslashes;
  end;
end;
...
begin
  ...
  {$ENDIF}
    TMyConnection(Dest).HttpOptions.Assign(HttpOptions);
  end;
  inherited;
end;
...
procedure TMyConnection.SetHttpOptions(Value: THttpOptions);
begin
...
procedure TMyConnection.DoConnect;
begin
  if IConnection <> nil then
    IConnection.HttpOptions := FHttpOptions;
...
{$ENDIF}
  ...
  ConnectionParameters.HttpOptions.Assign(HttpOptions);
{$IFDEF HAVE_OPENSSL}
  ConnectionParameters.SSL_Cipher := SSLOptions.ChipherList;
...
begin
  ...
  MySQLConnection.HttpOptions := FHttpOptions;
  ...
  {$ENDIF}
    ...
    MySQLConnection.SetProp(prCheckBackslashes, Options.FCheckBackslashes);

  {$IFDEF HAVE_OPENSSL}
    MySQLConnection.SetProp(prSSL_Cipher, SSLOptions.ChipherList);
...
begin
  ...
  FTableName := MySQLInfo.NormalizeName(Trim(Value), False, True);
end;
...
function TCustomMyTable.SQLSetOrderBy(SQLText: _string; Fields: _string): _string;
begin
  Result := inherited SQLSetOrderBy(SQLText, Fields);
  FOrderFields := Fields;
end;
...
begin
  ...
  UQ.SQL.Text := 'SHOW INDEX FROM ' + MySQLInfo.NormalizeName(TableName);
...
procedure TCustomMyTable.CheckSQL;
begin
  PrepareSQL;
end;
...
procedure TCustomMyStoredProc.DoBeforeExecute;
begin
...
begin
  ...
  if Dest is TMyCommand then begin
    TMyCommand(Dest).BeforeExecute := BeforeExecute;
...
procedure TMyConnectionSSLOptions.SetCACert(const Value: string);
begin
...
procedure TMyConnectionSSLOptions.SetCipherList(const Value: string);
begin
  if FCipherList <> Value then begin
    TCustomMyConnection(FOwner).CheckInactive;
    FCipherList := Value;
    if TCustomMyConnection(FOwner).IConnection <> nil then
      TCustomMyConnection(FOwner).IConnection.SetProp(prSSL_Cipher, Value);
  end;
end;
...
function TMyMetaData.GetConnection: TCustomMyConnection;
begin
  Result := TCustomMyConnection(inherited Connection);
end;

procedure TMyMetaData.SetConnection(Value: TCustomMyConnection);
begin
  inherited Connection := Value;
end;
