...
{$IFDEF VER6P}
  Types,
{$ENDIF}
  SysUtils, Classes, CRTypes, CRFunctions, MemUtils;
...
begin
  ...
  if {$IFDEF VER16P}FormatSettings.{$ENDIF}DecimalSeparator <> '.' then begin
    l := Pos({$IFDEF VER16P}FormatSettings.{$ENDIF}DecimalSeparator, Result);
    if l <> 0 then
      Result[l] := '.';
  end;
end;
...
{$ENDIF}
  ...
  begin
    ...
    {$ELSE}
      Len := CRFunctions.UnicodeToUtf8(pBnd.buffer, pBnd.buffer_length, PWideChar(ws), Length(ws));
    {$ENDIF}
