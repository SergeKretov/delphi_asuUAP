...
uses
  Math, MemUtils, CRFunctions, CRParser, DAConsts, DASQLMonitor,
{$IFNDEF UNIDACPRO}
  MyConsts, MyParser;
{$ELSE}
        ...
        if DefaultExpressionOldBehavior then begin
          ...
          case Field.DataType of
            ...
            ftFloat, ftBCD{$IFDEF VER6P}, ftFMTBCD{$ENDIF}:
              Field.DefaultExpression := StringReplace(Def, '.', {$IFDEF VER16P}FormatSettings.{$ENDIF}DecimalSeparator, [rfReplaceAll]);
            ftDateTime:
  ...
  if (not IsFetchAll and FDataSet.Options.QueryRecCount) // DefaultResultSet with FetchAll = False
    ...
    try
      if Parser.ToLexem(lxSELECT) <> lcEnd then begin
        ...
        if (Lexem = lxFROM) or (Parser.ToLexem(lxFROM) <> lcEnd) then begin
                ...
                {$ELSE}
                  ...
                  while Piece <> nil do begin
                    BinToHex(PtrOffset(Piece, sizeof(TPieceHeader)),
                      {$IFDEF VER12P}PWideChar{$ELSE}PAnsiChar{$ENDIF}({$IFDEF VER12P}PWideChar{$ELSE}PAnsiChar{$ENDIF}(SValue) + sbOffset), Piece.Used);
