...
uses
  Classes, CRTypes, CRFunctions, MyAccess, MyClasses;
...
uses
  DB, MyConsts, DAConsts, SysUtils, DBAccess, CRAccess, MyCall{$IFDEF VER6P}, StrUtils{$ENDIF};
...
begin
  ...
  while not FQuery.Eof do begin
    if not (((MsgTypeFld.AsString = 'status') or (MsgTypeFld.AsString = 'sta'))
      and (MsgTextFld.AsString = 'OK'))
      and not (AnsiSameText(MsgTypeFld.AsString, 'warning')
      and AnsiContainsText(MsgTextFld.AsString, 'deprecated and will be removed')) then begin
      if Msg <> '' then
        Msg := Msg + #$D#$A;
...
var
  ...
  begin
    for i := 0 to TablesList.Count - 1 do begin
      TableName := MySQLInfo.QuoteIfNeed(TablesList[i]);
      ...
      s :=
        ...
        'FROM ' + MySQLInfo.QuoteIfNeed(TableName);
  ...
  begin
    IConnection := TMySQLConnection(TDBAccessUtils.GetIConnection(Connection));
    if (IConnection.ServerPrimaryVer = 5) and (IConnection.ServerMinorVer >= 5) then
      DatabaseError('The BACKUP TABLE statement is deprecated and is removed in MySQL 5.5 and higher.' + #13#10 +
      'Use the bmText mode instead of bmBinary');

    TableNames := '';
    for i := 0 to TablesList.Count - 1 do begin
      if i = 0 then
        TableNames := MySQLInfo.QuoteIfNeed(TablesList[i])
      else
        TableNames := TableNames + ', ' + MySQLInfo.QuoteIfNeed(TablesList[i]);
    ...
    FQuery.Execute;
    try
      TableNames := MySQLInfo.NamesFromList(TablesList);
...
var
  ...
  begin
    for i := 0 to TablesList.Count - 1 do begin
      ...
      s :=
        ...
        GetDupString + 'INTO TABLE ' + MySQLInfo.QuoteIfNeed(TableName) + #$D#$A +
  ...
  begin
    FQuery.SQL.Text := 'RESTORE TABLE ' + MySQLInfo.NamesFromList(TablesList) + ' FROM "' + DuplicateBackSlash(IncludeTrailingBackslash(Path)) + '"';
    FQuery.Execute;
