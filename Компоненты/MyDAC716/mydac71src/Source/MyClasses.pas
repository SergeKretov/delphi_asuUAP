...
{$ENDIF}
{$IFDEF CLR}
  System.Runtime.InteropServices, System.Text, System.Globalization,
  Borland.Vcl.TypInfo,
{$ELSE}
  CLRClasses,
{$ENDIF}
...
{$IFDEF HAVE_OPENSSL}
  prSSL_Cipher      = prMySQLBase + 20; // string
...
{$IFDEF HAVE_DIRECT}
  ...
  {$ENDIF}
{$ENDIF}
  SysConst, CRFunctions, MemUtils, DAConsts, Math,
{$IFNDEF UNIDACPRO}
  ...
  begin
    ...
    if not UseUnicode then
      ...
      if Blob.IsUnicode then
        ...
        Result := AnsiToUtf8(EscapeAndQuoteStr(PAnsiChar(Blob.FirstPiece) + sizeof(TPieceHeader), Blob.FirstPiece.Used, NeedBackslashes, Charset));
  end;
      ...
      begin {uncompr.c} // Does not use SendBlobAsIs for performance reason
        if not UseUnicode then
          ...
          if Blob.IsUnicode then
            ...
            Result := AnsiToUTF8(EscapeAndQuoteStr(Blob.AsAnsiString, Blob.Size, NeedBackslashes, Charset));
      end;
    end;
  {$ELSE}
    SendBlobAsIs(Blob);
...
{$ENDIF}
  ...
  {$ENDIF}
    while i < Length(Result) do begin
      if Result[i] = {$IFDEF VER16P}FormatSettings.{$ENDIF}DecimalSeparator then begin
        Result[i] := '.';
        Break;
      end;
      Inc(i);
...
procedure AppendValueToSQL(
  ...
  procedure DivMod(Dividend: Int64; Divisor: integer;
  ...
  begin
    case DataType of
      ...
      dtTime: begin
        ...
        DivMod(SecondsAll, 60{Min/Hour} * 60{Sec/Min}, HourCount, SecCount);
  ...
  {$IFDEF HAVE_OPENSSL}
    if FProtocol = mpSSL then begin
      ...
      FMySQLAPI.mysql_ssl_set(FMySQL, PAnsiChar(AnsiString(FSSL_Key)),
        ...
        nil, PAnsiChar(AnsiString(FSSL_Cipher)));
    end
    else
  {$ENDIF}
  {$IFNDEF LITE}
...
procedure TMySQLCommand.GetSPParams(const Name: _string; out IsFunc: boolean;
  ...
  begin
    ...
    try
      ...
      RecordSet.FCommand.SQL := 'SELECT type, returns, param_list FROM mysql.proc WHERE (name = ''' + ProcName +
        ''') AND (db = ''' + Database + ''')';
      RecordSet.SetProp(prFlatBuffers, False);
      RecordSet.SetProp(prLongStrings, True);
      RecordSet.SetProp(prFetchAll, True);
      ...
      if RecordSet.RecordCount = 0 then begin
        RecordSet.Close;
        RecordSet.FCommand.SQL := 'SELECT type, returns, param_list FROM mysql.proc WHERE (name = ''' + ProcName +
        ''') AND (LOWER(db) = ''' + AnsiLowerCase(Database) + ''')';
        RecordSet.Open;
      end;
  ...
  begin
    ...
    try
      ...
      RecordSet.FCommand.SQL := 'SELECT ROUTINE_TYPE FROM INFORMATION_SCHEMA.ROUTINES '+
        'WHERE (ROUTINE_NAME = ''' + ProcName + ''') AND (ROUTINE_SCHEMA = ''' +
        Database + ''')';
      RecordSet.SetProp(prFlatBuffers, False);
      RecordSet.SetProp(prLongStrings, True);
      RecordSet.SetProp(prFetchAll, True);
  ...
  var
    ...
    {$ENDIF}
      ...
      begin
        ...
        if FConnection.FUseUnicode then begin
          u := UTF8Encode(copy(FSQL, Prev + 1, Length(FSQL) - Prev));
          SQL.Append(u);
        end
        else
...
function TMySQLInfo.NamesFromList(List: _TStrings): _string;
var
  i: integer;
begin
  Result := '';
...
procedure TMySQLInfo.NamesToList(Value: _string; List: _TStrings);
var
  St: _string;
  i: integer;
begin
...
var
  ...
  begin
    ...
    begin
      Parser.ToBegin;
      i := Parser.ToLexem(lxPROCEDURE, True);
      if i = lcEnd then begin
        Parser.ToBegin;
        i := Parser.ToLexem(lxFOR, True);
        if i = lcEnd then begin
          Parser.ToBegin;
          i := Parser.ToLexem(lxLOCK, True);
        end;
      end;
      if i <> lcEnd then begin
    ...
    begin
            ...
            if (currVal = byte('e')) or (currVal = byte('E')) then begin
              ...
              if isNegative then
                if point = end2 then // if without point
                  d := - result1 * IntPower(Base10, e)
                else
                  d := - result1 * IntPower(Base10, e - (off - point - 1))
              else
                if point = end2 then // if without point
                  d := result1 * IntPower(Base10, e)
                else
        ...
        if (currVal < byte('0')) or (currVal > byte('9')) then begin
          if (point = end2) and (currVal = byte('.')) then
            ...
            if (currVal = byte('e')) or (currVal = byte('E')) then begin
              ...
              if isNegative then
                if point = end2 then // if without point
                  d := - (result1 * IntPower(multiplerForLen1, e) + result2 * IntPower(Base10, e))
                else
                  d := - (result1 * IntPower(multiplerForLen1, e - (off - point - 1)) + result2 * IntPower(Base10, e - (off - point - 1)))
              else
                if point = end2 then // if without point
                  d := (result1 * IntPower(multiplerForLen1, e) + result2 * IntPower(Base10, e))
                else
      ...
      if end2 <> end1 then  // long float (length is more than PartLen)
        if result2 = 0 then  // finishes by 00000000
          if point <= end1 then
            d := result1 * IntPower(Base10, point - start - (end1- start))
          else
            if point = end2 then  // without floating point
              d := result1 * IntPower(Base10, length - (end1 - start))
            else // (point > end1) and (point < end2)
              d := result1 * IntPower(Base10, point - end1 - 1)
        else
          d := (result1 * IntPower(Base10, end2 - end1) + result2) * IntPower(Base10, - (end2 - point))
      else
          d := result1 * IntPower(Base10, point - end1);
  ...
  {$ELSE}
    ...
    begin
      ...
      if CorrectDecimalSeparator then begin
        ...
        while i <> ValueArrOffEnd do begin
          if ValueArr[i] = '.' then
            ValueArr[i] := AnsiChar({$IFDEF VER16P}FormatSettings.{$ENDIF}DecimalSeparator);
          Inc(i);
        end;
      end;
...
begin
  ...
  try
    ...
    try
      ...
      CorrectDecimalSeparator := {$IFDEF VER16P}FormatSettings.{$ENDIF}DecimalSeparator <> '.';
...
begin
  ...
  for i := 0 to Columns.Count - 1 do
    if i = 0 then
      ColumnList := MySQLInfo.QuoteIfNeed(Columns[i].Name)
    else
