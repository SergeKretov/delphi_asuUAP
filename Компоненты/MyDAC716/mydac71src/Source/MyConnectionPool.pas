...
uses
  Classes, CRConnectionPool, CRAccess, CRTypes, MemData, CRVio, DASQLMonitor,
{$IFDEF VER6P}
  Variants,
{$ENDIF}
{$IFNDEF UNIDACPRO}
...
begin
  ...
  {$ENDIF}
    TMyConnectionParameters(Dest).SSL_Cipher := SSL_Cipher;
...
begin
  ...
  {$IFDEF HAVE_OPENSSL}
    Result.SetProp(prSSL_Cipher, TMyConnectionParameters(ConnectionParameters).SSL_Cipher);
