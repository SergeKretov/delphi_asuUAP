...
//  Copyright (c) 2004-2012 Devart. All right reserved.
//  Query component
//  Created:
//  Last modified:
//////////////////////////////////////////////////

unit FR_MyDACQueryParam;
...
uses
  ...
  StdCtrls, FR_Ctrls, ExtCtrls, DB, MyAccess, FR_MyDACQuery;

type
  TfrMyDACParamsForm = class(TForm)
    GroupBox1: TGroupBox;
    Label2: TLabel;
    ParamsLB: TListBox;
    TypeCB: TComboBox;
...
function TfrMyDACParamsForm.CurParam: Integer;
var
  i: Integer;
  s: String;
begin
...
procedure TfrMyDACParamsForm.FormShow(Sender: TObject);
var
  i: Integer;
begin
...
procedure TfrMyDACParamsForm.FormHide(Sender: TObject);
begin
  if ValueE.Enabled then ValueEExit(nil);
end;

procedure TfrMyDACParamsForm.ParamsLBClick(Sender: TObject);
var
  i: Integer;
begin
  TypeCB.ItemIndex := -1;
...
procedure TfrMyDACParamsForm.ValueEExit(Sender: TObject);
begin
  QueryComp.ParamText[CurParam] := ValueE.Text;
end;

procedure TfrMyDACParamsForm.TypeCBChange(Sender: TObject);
begin
...
procedure TfrMyDACParamsForm.ValueRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Enabled := True;
...
procedure TfrMyDACParamsForm.AssignRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Text := '';
  ValueE.Enabled := False;
...
procedure TfrMyDACParamsForm.Localize;
begin
...
procedure TfrMyDACParamsForm.FormCreate(Sender: TObject);
begin
  Localize;
end;

procedure TfrMyDACParamsForm.VarSBClick(Sender: TObject);
begin
